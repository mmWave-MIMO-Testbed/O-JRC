...
class mimo_ofdm_joint_TRX(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.settings = Qt.QSettings("GNU Radio", "mimo_ofdm_joint_TRX")
        ...
        self.radar_log_file = radar_log_file = "/home/hostpc-usrp/temp/radar_log.csv"
        self.radar_chan_file = radar_chan_file = "/home/hostpc-usrp/temp/radar_chan.csv"
        ...
        self.chan_est_file = chan_est_file = "/home/hostpc-usrp/temp/chan_est.csv"
        ...
        self.amp_tx4 = amp_tx4 = 0.9
        self.amp_tx3 = amp_tx3 = 0.9
        self.amp_tx2 = amp_tx2 = 0.9
        self.amp_rx2 = amp_rx2 = 1.12
        ...
        self._use_radar_streams_labels = ['OFF', 'ON']
        # Create the combo box
        self._use_radar_streams_tool_bar = Qt.QToolBar(self)
        self._use_radar_streams_tool_bar.addWidget(Qt.QLabel('Radar    \nStreams' + ": "))
        ...
        self.top_grid_layout.addWidget(self._use_radar_streams_tool_bar, 20, 0, 1, 2)
        ...
        for c in range(2, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._phase_tx4_range = Range(-2*cmath.pi, 2*cmath.pi, 0.01, 2.2, 200)
        ...
        for c in range(6, 8):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._phase_tx3_range = Range(-2*cmath.pi, 2*cmath.pi, 0.01, 1.5, 200)
        ...
        for c in range(2, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._phase_rx2_range = Range(-2*cmath.pi, 2*cmath.pi, 0.01, -0.6, 200)
        ...
        for c in range(0, 2):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._amp_tx4_range = Range(0, 10, 0.01, 0.9, 200)
        ...
        for c in range(6, 8):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._amp_tx3_range = Range(0, 10, 0.01, 0.9, 200)
        ...
        for c in range(4, 6):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._amp_tx2_range = Range(0, 10, 0.01, 0.9, 200)
        ...
        for c in range(2, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        self._amp_rx2_range = Range(0, 10, 0.01, 1.12, 200)
        ...
        # Create the radio buttons
        self.top_grid_layout.addWidget(self._phased_steering_tool_bar, 20, 2, 1, 2)
        for r in range(20, 21):
            self.top_grid_layout.setRowStretch(r, 1)
        for c in range(2, 4):
            self.top_grid_layout.setColumnStretch(c, 1)
        ...
        self.mimo_ofdm_jrc_mimo_precoder_0 = mimo_ofdm_jrc.mimo_precoder(fft_len, N_tx, 1, ofdm_config.data_subcarriers, ofdm_config.pilot_subcarriers, ofdm_config.pilot_symbols, ofdm_config.l_stf_ltf_64, ofdm_config.ltf_mapped_sc__ss_sym, chan_est_file, freq_smoothing, radar_log_file, radar_aided, False, use_radar_streams, "packet_len",  False)
        ...
        self.mimo_ofdm_jrc_gui_heatmap_plot_0 = mimo_ofdm_jrc.gui_heatmap_plot(N_tx*N_rx*interp_factor_angle, 100, "Angle", "Range (m)", 'Range-Angle Image', angle_axis, np.linspace(0, 3e8*fft_len/(2*samp_rate), fft_len*interp_factor), 9, [-70, 70, 10], [0, 20, 4], False, True, "packet_len")
...
def main(top_block_cls=mimo_ofdm_joint_TRX, options=None):
