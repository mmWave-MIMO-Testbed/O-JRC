...
class BfRamTableView():
    def __init__(self, parent, main_view, dev, *args, **kwargs):
        ...
        self.bra = actions.bf_ram_actions.BfRamActions(main_view.gui_data)

        self.selected_index = {}
        self.selected_index['th'] = self.main_view.gui_data.read_register('beam_tx_cur_h')
        self.selected_index['tv'] = self.main_view.gui_data.read_register('beam_tx_cur_v')
        self.selected_index['rh'] = self.main_view.gui_data.read_register('beam_rx_cur_h')
        self.selected_index['rv'] = self.main_view.gui_data.read_register('beam_rx_cur_v')
        self.selected_index['nth'] = self.main_view.gui_data.read_register('beam_tx_next_h')
        self.selected_index['ntv'] = self.main_view.gui_data.read_register('beam_tx_next_v')
        self.selected_index['nrh'] = self.main_view.gui_data.read_register('beam_rx_next_h')
        self.selected_index['nrv'] = self.main_view.gui_data.read_register('beam_rx_next_v')
        ...
        self.ram_value = {}
        self.ram_value = self.main_view.gui_data.read_ram_complete('ram', 'compact')

        self.fv = BfRamTableFieldView(self, parent, self.main_view, dev, self.ram_value, 10, self.value_modified, *args, **kwargs)
        self.bv = BfRamTableByteView(self, parent, self.main_view, dev, self.ram_value, col_width=8, value_changed_callback=self.value_modified, *args, **kwargs)
        self.cv = BfRamTableCompactView(self, parent, self.main_view, dev, self.ram_value, value_changed_callback=self.value_modified, *args, **kwargs)
    ...
    def open_table_selector(self):
        ...
        if filename != None:
            ram_file_selector = RamFileSelector.RamFileSelector(self.parent, filename.name, filter=['RAM'], select_callback=self.ram_table_selected)

    def ram_table_selected(self, filename, table_id):
        self.main_view.gui_data.ram_fill(table_id, filename)
        self.read_ram()
...
class BfRamTableCompactView():
    def __init__(self, owner, parent, main_view, dev, ram_value, col_width=15, value_changed_callback=None, *args, **kwargs):
        self.owner = owner
        self.parent = parent
        self.main_view = main_view
        self.dev = dev
        ...
        self.number_of_bytes = self.main_view.gui_data.get_register_map_regs()['ram']['length']
    ...
    def write_changes(self):
        ...
        if any_rows_modified:
            ...
            for rowid in modified_rows:
                ...
                if value != None:
                    self.main_view.gui_data.write_ram('ram', int(rowid), value)
...
class BfRamTableFieldView():
    def __init__(self, owner, parent, main_view, dev, ram_value, col_width=15, value_changed_callback=None, *args, **kwargs):
        ...
        fields = list(self.main_view.gui_data.get_register_fields('ram'))
        self.field_details = [None]*len(fields)
        n = 0
        for field in fields:
            reg_field = self.main_view.gui_data.get_register_field('ram', field)
            lsb = reg_field['Lsb']
            msb = reg_field['Msb']
    ...
    def enter(self, event):
        ...
        if self.pointed_column != column or self.pointed_row != row:
            ...
            if row == '':
                self.toolTip = tooltip.ToolTip(self.main_table, text=self.field_names[int(column.replace('#',''))-1]+'['+str(self.field_sizes[int(column.replace('#',''))-1])+' bits]')
            else:
                self.toolTip = tooltip.ToolTip(self.main_table, text='(Index: ' + row + ') \n' + self.field_names[int(column.replace('#',''))-1]+'['+str(self.field_sizes[int(column.replace('#',''))-1])+' bits]')
    ...
    def reload_ram(self, ram_value=None):
        if ram_value == None:
            self.ram_value = self.main_view.gui_data('ram', 'compact')
