...
class GainView():
    ...
    def __init__(self, parent, gui_handler, dev, mode, *args, **kwargs):
        ...
        parent.bind("<FocusIn>", self._handle_focus)
    ...
    def _add_gain_adjust_frame(self):
        ...
        if self.mode == 'RX':
            self.selected_gain_index_v = tk.IntVar(value=self.host.chip.rx.curr_gain_index(self.dev, 'v'))
        else:
            self.selected_gain_index_v = tk.IntVar(value=self.host.chip.tx.curr_gain_index(self.dev, 'v'))
        ...
        if self.mode == 'RX':
            self.selected_gain_index_h = tk.IntVar(value=self.host.chip.rx.curr_gain_index(self.dev, 'h'))
        else:
            self.selected_gain_index_h = tk.IntVar(value=self.host.chip.tx.curr_gain_index(self.dev, 'h'))
