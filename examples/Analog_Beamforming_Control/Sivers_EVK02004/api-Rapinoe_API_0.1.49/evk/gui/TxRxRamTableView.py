...
class TxRxRamTableView():
    def __init__(self, parent, gui_handler, dev, txrx_ram, *args, **kwargs):
        ...
        if txrx_ram.lower() == 'tx_ram':
            self.spinbox_selected_index.set(self.gui_handler.host.chip.tx.curr_gain_index(dev, 'h'))
        else:
            self.spinbox_selected_index.set(self.gui_handler.host.chip.rx.curr_gain_index(dev, 'h'))
    ...
    def polarization_selected(self, event):
        ...
        if self.txrx_ram.lower() == 'tx_ram':
            self.spinbox_selected_index.set(self.gui_handler.host.chip.tx.curr_gain_index(self.dev, p.lower()))
        else:
            self.spinbox_selected_index.set(self.gui_handler.host.chip.rx.curr_gain_index(self.dev, p.lower()))
    ...
    def ram_table_selected(self, filename, table_id):
        if self.get_selected_polarization() == 'v' and self.txrx_ram == 'RX_RAM':
            ...
            tx_v_gain_index = self.gui_handler.host.chip.tx.curr_gain_index(self.dev, 'V')
            tx_h_gain_index = self.gui_handler.host.chip.tx.curr_gain_index(self.dev, 'H')
            rx_h_gain_index = self.gui_handler.host.chip.rx.curr_gain_index(self.dev, 'H')
