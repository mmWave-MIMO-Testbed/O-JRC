...
class GeneralView():
    ...
    def _add_init_frame(self):
        ...
        row_frame[3] = p.Page(init_frame)

        self.init_chip_button = tk.Button(row_frame[0], text='Chip', command=lambda: self._init('CHIP'), width=10)
        if self.host.chip._chip_info._chip_info[self.dev.id]['R-rev'] == '5':
            self.init_rcu_button = tk.Button(row_frame[0], text='RCU', command=lambda: self._init('RCU'), width=10)
            self.init_synth_button = tk.Button(row_frame[1], text='Synth', command=lambda: self._init('SYNTH'), width=10)
            self.init_adc_button = tk.Button(row_frame[1], text='ADC', command=lambda: self._init('ADC'), width=10)
            self.init_val_button = tk.Button(row_frame[2], text='VAL.', command=lambda: self._init('VALIDATION'), width=10)
            self.init_tx_button = tk.Button(row_frame[3], text='TX', command=lambda: self._init('TX'), width=10)
            self.init_rx_button = tk.Button(row_frame[3], text='RX', command=lambda: self._init('RX'), width=10)
            self.init_chip_button.pack(side='left', padx=4, pady=4, expand = True, fill = 'both')
            self.init_rcu_button.pack(side='left', padx=4, pady=4, expand = True, fill = 'both')
            ...
            self.init_val_button.pack(side='left', padx=4, pady=4, expand = False, fill = 'both')
    ...
    def _init(self, block):
        ...
        if block == 'CHIP':
            ...
            self.init_rcu_button.configure(fg='green')
        elif block == 'SYNTH':
    ...
    def _add_synth_frame(self):
        ...
        rev = int(self.host.chip.info(self.dev)['R-rev'])
        if rev >= 5:
    ...
    def _reset_button_colors(self):
        ...
        try:
            self.init_rcu_button.configure(fg='black')
        except:
            pass
