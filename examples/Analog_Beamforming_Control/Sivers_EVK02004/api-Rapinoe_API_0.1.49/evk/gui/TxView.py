...
class TxView():
    ...
    def _beam_set(self):
        ...
        self.ta.beam(self.dev, pol, beam_index)
    ...
    def _rf_gain_set(self):
        ...
        if rf_gain_index != None:
            ...
            self.ta.rf_gain(self.dev, gain_pol, rf_gain_index)
    ...
    def _bb_gain_set(self):
        ...
        if bb_v != None and bb_h != None:
            ...
            self.ta.bb_gain(self.dev, bb_v, bb_h)
    ...
    def setup_set(self):
        ...
        self.ta.setup(self.dev, mode, pol, latitude, ant_en_v, ant_en_h)
    ...
    def _calibrate(self):
        ...
        pdet_calib = self.selected_pdet_calib.get()
        ...
        if pdet_calib == 'No':
            ...
            self.host.gui_handler.TH.put(lambda : self.ta.pdet_dco_calibrate(self.dev, mode, tx_pol))
    ...
    def _add_calib_frame(self):
        ...
        row_frame[5] = p.Page(calib_frame)
        ...
        self.pdet_calib_label = tk.Label(row_frame[2], text='PDET Calib.')
        pdet_calib_value_list = ['Yes', 'No']
        self.selected_pdet_calib = tk.StringVar(value=pdet_calib_value_list[1])
        def on_pdet_calib_changed(pdet_calib):
            if pdet_calib.get() != str(self.set_calib_setting['pdet_calib']):
                self.calibrate_button.configure(fg='red')
            if pdet_calib.get() == 'Yes':
                self.selected_cross_pol_calib.set('N/A')
                self.cross_pol_calib_selector.configure(state='disabled')
            else:
                self.selected_cross_pol_calib.set('Yes')
                self.cross_pol_calib_selector.configure(state='readonly')
        self.selected_pdet_calib.trace('w', lambda *_, pdet_calib=self.selected_pdet_calib: on_pdet_calib_changed(pdet_calib))
        self.pdet_calib_selector = ttk.Combobox(row_frame[2], values=pdet_calib_value_list, width=10, state='readonly', textvariable=self.selected_pdet_calib)

        self.cross_pol_calib_label = tk.Label(row_frame[3], text='Cross Pol. Calib.')
        ...
        self.cross_pol_calib_selector = ttk.Combobox(row_frame[3], values=cross_pol_calib_value_list, width=10, state='readonly', textvariable=self.selected_cross_pol_calib)


        separator = ttk.Separator(row_frame[4], orient='horizontal')

        self.calibrate_button = tk.Button(row_frame[5], text='Calibrate', command=self._calibrate)
        ...
        self.pdet_calib_label.pack(side='left', padx=4, pady=4)
        self.pdet_calib_selector.pack(side='right', padx=4, pady=4)
        row_frame[2].pack(side = 'top', expand = True, fill = 'both')
        ...
        self.set_calib_setting = {'tx_pol':None, 'cross_pol':None, 'pdet_calib':None}
