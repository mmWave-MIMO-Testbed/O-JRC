...
class RxDco():
    ...
    def __init__(self, rx, ram, adc):
        ...
        self.field_index = {'bbrx_i_dco_drv':14,
                            'bbrx_i_dco_input':13,
                            'bbrx_q_dco_drv':12,
                            'bbrx_q_dco_input':11,
                            'bbrx_i_pga2_gain':10,
                            'bbrx_i_filter_gain':9,
                            'bbrx_q_pga2_gain':8,
                            'bbrx_q_filter_gain':7,
                            'bbrx_iq_filter_gain':6,
                            'bbrx_iq_pga1_gain':5,
                            'com_gain_rx_vga':4,
                            'bf_gain_rx_vga':3,
                            'bf_gain_lna':2,
    ...
    def _restore_settings(self, dev, pol, gain_index):
        ...
        exclude_fields = ['bbrx_i_dco_drv', 'bbrx_i_dco_input', 'bbrx_q_dco_drv', 'bbrx_q_dco_input', 'not_used']
        for field in list(self.field_index):
    ...
    def _calibration_setting_stage_2(self, dev, pol, gain_index, bf_gain_lna=0, bf_gain_rx_vga=0, bf_att_com=0x1f, com_gain_rx_vga=0, bbrx_filter_gain=0, bbrx_iq_filter_gain=0, bbrx_iq_pga1_gain=0, bbrx_pga2_gain=2, bbrx_dco_input=0x0):
    ...
    def report(self, dev, pol, indentation=0, return_results=True, printit=True):
        bb_rx_en_dco_backup = self._spi.rd(dev, 'bb_rx_en_dco')
        bb_rx_en_dco = bb_rx_en_dco_backup
        if pol == 'V':
            bb_rx_en_dco = bb_rx_en_dco | 0xf0
        else:
            bb_rx_en_dco = bb_rx_en_dco | 0x0f
        self._spi.wr(dev, 'bb_rx_en_dco', bb_rx_en_dco)
        ...
        self._spi.wr(dev, 'bb_rx_en_dco', bb_rx_en_dco_backup)
        ...
        if printit:
            ...
            evk_logger.evk_logger.log_info('')
        if return_results:
            return {'dco_pga1_i':dco_i1_V, 'dco_pga1_q':dco_q1_V, 'dco_pga2_i':dco_i2_V, 'dco_pga2_q':dco_q2_V}
    ...
    def _read_dco(self, dev, pol, chan, stage):
        ...
        """
        return self._adc.get_data(dev, self.amux_src[pol.upper()+chan.upper()+str(stage)], reps=10)
    ...
    def _calib(self, dev, pol, chan, stage, gain_index):
        START = 0
        MID = 1
        END = 2
        meas_retry = 3
        ...
        while (not dco_reg[START] == dco_reg[MID]) and (not dco_reg[END] == dco_reg[MID]):
            ...
            dco_diff[START] = self._read_dco(dev, pol, chan, 2)
            self._set_field(dev, pol, gain_index, bbrx_dco_drv, dco_reg[MID])
            dco_diff[MID] = self._read_dco(dev, pol, chan, 2)
            self._set_field(dev, pol, gain_index, bbrx_dco_drv, dco_reg[END])
            dco_diff[END] = self._read_dco(dev, pol, chan, 2)
    ...
    def calibrate(self, dev, pol, gain_index, return_results=True):
        ...
        bb_rx_en_dco = self._spi.rd(dev, 'bb_rx_en_dco')
        if pol == 'V':
            backup_bb_rx_en_dco = bb_rx_en_dco & 0x0f # Backup H section
            bb_rx_en_dco = 0
            bb_rx_en_dco = bb_rx_en_dco | 0xf0
        else:
            backup_bb_rx_en_dco = bb_rx_en_dco & 0xf0 # Backup V section
            bb_rx_en_dco = 0
            bb_rx_en_dco = bb_rx_en_dco | 0x0f
        ...
        bb_rx_en_dco = bb_rx_en_dco | backup_bb_rx_en_dco
        self._spi.wr(dev, 'bb_rx_en_dco', bb_rx_en_dco)
