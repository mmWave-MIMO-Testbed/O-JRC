...
class Temp():
    ...
    def __init__(self, spi):
        if self.__initialized != True:
            ...
            for dev_num in range(0,self._chip_info.get_num_devs()):
                ...
                self.conv_dec.update({dev: {'BG'  :  {'slope':(-960.886)/1024*1.2, 'offs':739.636},
                                            'SYNTH': {'slope':0.26, 'offs':15.644},
    ...
    @evk_logger.log_call
    def get(self, devs, src='BG', formatit='adc', printit=True, enable_activate='both'):
        if src in self.src_names:
            ...
            if formatit.lower() == 'temp' or formatit.lower() == 'volt':
                ...
                formatit_ = formatit
            if src.upper() == 'BG':
                adc_data = self.adc.get_data(devs,'BG Sense',formatit_)
            else:
