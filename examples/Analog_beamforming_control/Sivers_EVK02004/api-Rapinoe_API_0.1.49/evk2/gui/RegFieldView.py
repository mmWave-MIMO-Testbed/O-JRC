...
class RegFieldView():
    ...
    def __init__(self, parent, update_callback, gui_data, dev, reg, wr_button_text='Write changes', rd_button_text=None, *args, **kwargs):
        ...
        fields = gui_data.rem_evk.get_register_fields(reg)
        for field in fields:
            self.field_view.append(FieldView(self.fieldbox, self.update_callback, gui_data, dev, reg, field, owner=self))
...
class FieldView():
    ...
    def __init__(self, parent, update_callback, gui_data, dev, reg, field, *args, **kwargs):
        self.gui_data = gui_data
        self.dev = dev
        self.reg = reg
        self.field = field
        ...
        field_details = gui_data.rem_evk.get_register_field(reg, field)
        field_bit_label_text = str(field_details)
        field_bit_label = tk.Label(field_row, text=field_bit_label_text)

        self.mask = 0
        for bit in range(field_details['Lsb'], field_details['Msb']+1):
            self.mask = self.mask + (1 << bit)

        self.current_value = (gui_data.read_register(self.reg) & self.mask) >> field_details['Lsb']
        ...
        self.num_of_bits = field_details['Msb'] - field_details['Lsb'] + 1
    ...
    def write_new_value(self, event):
        ...
        if entry_value != self.current_value:
            ...
            if 'ram' in self.reg:
                self.gui_data.write_ram(self.reg, self.owner.row, value)
            else:
                self.gui_data.write_register(self.reg, value)
            if not self.update_callback == None:
                self.update_callback()

    def read_reg_value(self):
        if 'ram' in self.reg:
            field_details = self.gui_data.rem_evk.get_register_field(self.reg, self.field)
            current_value = (self.gui_data.read_ram(self.reg, self.owner.row) & self.mask) >> field_details['Lsb']
        else:
            field_details = self.gui_data.rem_evk.get_register_field(self.reg, self.field)
            current_value = (self.gui_data.read_register(self.reg) & self.mask) >> field_details['Lsb']
