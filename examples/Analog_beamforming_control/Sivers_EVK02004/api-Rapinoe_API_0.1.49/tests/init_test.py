...
#host.pll.setup()  # Lock the MB2-VCXO to 245.76 MHz
#vcxo_freq=host.pll.setup()['vcxo_freq'] #Read back the exact frequency (typically 245759996.9474969 Hz)
host.chip.ref_clk.set(rap1, vcxo_freq) #Set the Rapinoe clock to vcxo_freq.

#RFIC initialisation
host.chip.init(rap1, 'CHIP')
host.chip.init(rap1, 'SYNTH')
host.chip.init(rap1, 'VALIDATION')
host.chip.init(rap1, 'ADC')
host.chip.init(rap1, 'TX', printit=False)

# Synth setup with freq_rff
freq_rff = 24e9 # LO frequency. The script is for IF mode. So RF will be IF freq + freq_rff
host.chip.synth.setup(rap1)
host.chip.synth.set(rap1,freq_rff, frac_mode=True, sd_order=2, printit=True)
host.chip.ram.fill(rap1, '24GHz') # Loads beam for 24GHz from file to RAM. Depending on the RF value a different beambook should be loaded

#Enabling 16 elements on V pol
host.chip.tx.setup(rap1, mode, tpol, ant_en_v=0x0000, ant_en_h=0xFFFF) # for H pol or dual pol write appropriate values for ant_en_h. 0xFFFF is for all 16 paths (each bit is for one path). for ex, if you want just one path write 0x0001. 
...
host.chip.trx.mode(rap1,tpol)
host.chip.tx.beam(rap1, 5, tpol)
host.chip.tx.gain_rf(rap1, 0, tpol)

#calibrating for H and V pol separately
#host.chip.tx.dco.calibrate(rap0, mode, pol1)
host.chip.tx.dco.calibrate(rap1, mode, pol2) # uncomment this cmd and comment the line above if H pol is used or uncomment both lines if dual pol is used (two DCO calibrations are needed then)
