...
class TxDco():
    ...
    def _backup_settings(self, dev):
        ...
        reg_list = ['ssw_cfg_on_bf_en_tx_v', 'ssw_cfg_on_bf_en_tx_h', 'bb_tx_config_v', 'bb_tx_config_h', 'com_bias_lo_trim', \
                    'com_bias_tx_trim', 'com_bias_rx_trim', 'bb_rx_config_dco_v', 'bb_rx_config_dco_h', 'en_rx_v_override', 'en_rx_h_override', \
                    'bb_rx_config_v', 'bb_rx_config_h', 'ssw_cfg_on_sel_rx', 'ssw_cfg_on_en_rx_h', 'ssw_cfg_on_en_rx_v', \
                    'bf_en_rx_v_override', 'bf_en_rx_h_override', 'sel_rx_override', \
    ...
    def _pre_calibration_settings(self, dev, pol, rx_pol, gain_index=0):
        ...
        self._spi.wrrd(dev, 'com_bias_lo_trim', 0x6666)
        self._spi.wrrd(dev, 'com_bias_tx_trim', 0x242422)
        self._spi.wrrd(dev, 'com_bias_rx_trim', 0x606)
    ...
    def _rx_setup(self, dev, pol, rx_pol, mode):
        ...
        if mode == 'bb':
            #rx_gain_base =  0xF981F87B080000000000 # bbrx_iq_pga1_gain 0
            #rx_gain_base = 0xF981F97B080000000000 # bbrx_iq_pga1_gain 1
            #rx_gain_base = 0xF981FA7B080000000000 # bbrx_iq_pga1_gain 2
            #rx_gain_base = 0xF981FB7B080000000000 # bbrx_iq_pga1_gain 3
            rx_gain_base = 0xF981FC7B080000000000 # bbrx_iq_pga1_gain 4 #****
            #rx_gain_base = 0xF981FD7B080000000000 # bbrx_iq_pga1_gain 5
            #rx_gain_base = 0xF981FE7B080000000000 # bbrx_iq_pga1_gain 6
            #rx_gain_base = 0xF981FF7B080000000000 # bbrx_iq_pga1_gain 7
        else:
            rx_gain_base = 0xF981FF7B080000000000 # bbrx_iq_pga1_gain 7
        ...
        self._spi.clr(dev, 'ssw_cfg_on_sel_rx', 0x44)
        self._chip.rx.dco.calibrate_stage_1(dev, rx_pol.upper(), 0)

    def _select_pga1_gain_rx(self, dev, pol, rx_pol, mode):
        evk_logger.evk_logger.log_info('Selecting com_gain_rx_vga')
        reduction = 0.04

        rx_gain_base = 0xF980007B080000000000 # bbrx_iq_pga1_gain 7
        meas_point_i = 0 #int(self.max_reg_value / 2)
        meas_point_q = 0 #int(self.max_reg_value / 2)
        min_lo_leakage_magn = 1000000
        lo_leakage_magn = self._meas_lo_leakage_magn(dev, 0, 0, pol, rx_pol=rx_pol, n=4)
        if lo_leakage_magn < min_lo_leakage_magn:
            min_lo_leakage_magn = lo_leakage_magn
            meas_point_i = 0
            meas_point_q = 0
        lo_leakage_magn = self._meas_lo_leakage_magn(dev, 0, self.max_reg_value, pol, rx_pol=rx_pol, n=4)
        if lo_leakage_magn < min_lo_leakage_magn:
            min_lo_leakage_magn = lo_leakage_magn
            meas_point_i = self.max_reg_value
            meas_point_q = 0
        lo_leakage_magn = self._meas_lo_leakage_magn(dev, self.max_reg_value, self.max_reg_value, pol, rx_pol=rx_pol, n=4)
        if lo_leakage_magn < min_lo_leakage_magn:
            min_lo_leakage_magn = lo_leakage_magn
            meas_point_i = self.max_reg_value
            meas_point_q = self.max_reg_value
        lo_leakage_magn = self._meas_lo_leakage_magn(dev, self.max_reg_value, 0, pol, rx_pol=rx_pol, n=4)
        if lo_leakage_magn < min_lo_leakage_magn:
            min_lo_leakage_magn = lo_leakage_magn
            meas_point_i = 0
            meas_point_q = self.max_reg_value
        print('meas_point_i', meas_point_i)
        print('meas_point_q', meas_point_q)
        for bbrx_iq_pga1_gain in range(0x7, -1, -1):
            rx_gain = rx_gain_base + (bbrx_iq_pga1_gain << 56)
            self._chip.ram.wr(dev, 'rx_ram_'+rx_pol.lower(), 0, rx_gain)
            self._chip.rx.gain(dev, 0, 'R'+rx_pol.upper())
            if bbrx_iq_pga1_gain == 0x7:
                lo_leakage_magn0 = self._meas_lo_leakage_magn(dev, meas_point_q, meas_point_i, pol, rx_pol=rx_pol, n=4)
                evk_logger.evk_logger.log_info('bbrx_iq_pga1_gain: {} rx_gain: {} {}'.format(fhex(bbrx_iq_pga1_gain,1), fhex(rx_gain, 20), round(lo_leakage_magn0, 3), indentation=4))
                if lo_leakage_magn0 < 0.500:
                    return bbrx_iq_pga1_gain
            else:
                lo_leakage_magn = self._meas_lo_leakage_magn(dev, meas_point_q, meas_point_i, pol, rx_pol=rx_pol, n=4)
                if lo_leakage_magn > lo_leakage_magn0:
                    lo_leakage_magn0 = lo_leakage_magn
                evk_logger.evk_logger.log_info('bbrx_iq_pga1_gain: {} rx_gain: {} {}'.format(fhex(bbrx_iq_pga1_gain,1), fhex(rx_gain, 20), round(lo_leakage_magn, 3), indentation=4))
                if ((lo_leakage_magn0 - lo_leakage_magn) / lo_leakage_magn0) > reduction:
                    break
        return bbrx_iq_pga1_gain

    def _select_com_gain_rx(self, dev, pol, rx_pol, mode):
        evk_logger.evk_logger.log_info('Selecting com_gain_rx_vga')
        if pol == rx_pol:
            reduction = 0.05
        else:
        ...
        if mode == 'bb':
            #rx_gain_base = 0xF981F87B080000000000 # bbrx_iq_pga1_gain 0 (low gain)
            #rx_gain_base = 0xF981F97B080000000000 # bbrx_iq_pga1_gain 1
            #rx_gain_base = 0xF981FA7B080000000000 # bbrx_iq_pga1_gain 2
            #rx_gain_base = 0xF981FB7B080000000000 # bbrx_iq_pga1_gain 3
            rx_gain_base = 0xF981FC7B080000000000 # bbrx_iq_pga1_gain 4  #****
            #rx_gain_base = 0xF981FD7B080000000000 # bbrx_iq_pga1_gain 5
            #rx_gain_base = 0xF981FE7B080000000000 # bbrx_iq_pga1_gain 6
            #rx_gain_base = 0xF981FF7B080000000000 # bbrx_iq_pga1_gain 7 (high gain)
        else:
            rx_gain_base = 0xF981FF7B080000000000 # bbrx_iq_pga1_gain 7

        #rx_gain = self._chip.ram.rd(dev, 'rx_ram_'+rx_pol.lower(), 0)
        #rx_gain_base = rx_gain_base + (rx_gain & 0x7ffffffffffffff)
        meas_point_i = 0 #int(self.max_reg_value / 2)
        meas_point_q = 0 #int(self.max_reg_value / 2)
        min_lo_leakage_magn = 1000000
        lo_leakage_magn = self._meas_lo_leakage_magn(dev, 0, 0, pol, rx_pol=rx_pol, n=4)
        ...
        for com_gain_rx_vga in range(0x3f, -1, -1):
            rx_gain = rx_gain_base + (com_gain_rx_vga << 59)
            self._chip.ram.wr(dev, 'rx_ram_'+rx_pol.lower(), 0, rx_gain)
            self._chip.rx.gain(dev, 0, 'R'+rx_pol.upper())
            if com_gain_rx_vga == 0x3f:
                lo_leakage_magn0 = self._meas_lo_leakage_magn(dev, meas_point_q, meas_point_i, pol, rx_pol=rx_pol, n=4)
                evk_logger.evk_logger.log_info('com_gain_rx_vga: {} rx_gain: {} {}'.format(fhex(com_gain_rx_vga,2), fhex(rx_gain, 20), round(lo_leakage_magn0, 3), indentation=4))
                if lo_leakage_magn0 < 0.500:
                ...
                if lo_leakage_magn > lo_leakage_magn0:
                    lo_leakage_magn0 = lo_leakage_magn
                evk_logger.evk_logger.log_info('com_gain_rx_vga: {} rx_gain: {} {}'.format(fhex(com_gain_rx_vga,2), fhex(rx_gain, 20), round(lo_leakage_magn, 3), indentation=4))
                if ((lo_leakage_magn0 - lo_leakage_magn) / lo_leakage_magn0) > reduction:
                    break
        return com_gain_rx_vga


    def _select_rx_gain(self, dev, pol, rx_pol, mode):
        ...
        lo_leakage_magn = self._meas_lo_leakage_magn(dev, 0, 0, pol, rx_pol=rx_pol, n=4)
        if lo_leakage_magn > max_lo_leakage_magn:
    ...
    def _read_dco(self, dev, pol, chan, stage, reps=ADC_REP):
        ...
        """
        data = self._adc.get_data(dev, self.amux_src[pol.upper()+chan.upper()+str(stage)], reps=reps)
        return data

    def _read_tx_pdet(self, dev, pol, pdet_num, num_of_meas=1):
        pol = pol.upper()
    ...
    def _meas_lo_leakage_magn_pdet(self, dev, bbtx_dco_q, bbtx_dco_i, pol):
        ...
        return self._read_tx_pdet(dev, pol, 0)

    def _meas_lo_leakage_magn(self, dev, bbtx_dco_q, bbtx_dco_i, pol, rx_pol, test_data=False, reps=ADC_REP, n=1):
        """Measures LO leakage magnitude.
        ...
        if test_data:
            ...
            for i in range(n):
                # MEASURE LO leakage
                self._spi.wr(dev, 'com_bias_tx_trim', 0x0) #TX trim all off
                if pol != rx_pol:
                    self._switch_lb(dev, pol, False)
                i_dco_diff_o = self._read_dco(dev, rx_pol, 'I', DCO_STAGE, reps)
                q_dco_diff_o = self._read_dco(dev, rx_pol, 'Q', DCO_STAGE, reps)

                self._spi.wr(dev, 'com_bias_tx_trim', 0x242422)
                if pol != rx_pol:
                    self._switch_lb(dev, pol, True)

                i_dco_diff = self._read_dco(dev, rx_pol, 'I', DCO_STAGE, reps)
                #print('i_dco_diff: ', i_dco_diff)
                #print('i_dco_diff_o: ', i_dco_diff_o)
                q_dco_diff = self._read_dco(dev, rx_pol, 'Q', DCO_STAGE, reps)
                #print('q_dco_diff: ', q_dco_diff)
                #print('q_dco_diff_o: ', q_dco_diff_o)
    ...
    def calibrate_fine_tune(self, dev, pol, rx_pol, search_offset=2, bbtx_dco_reg=None):
        pol = pol.lower()
        ...
        for bbtx_dco_q in range(bbtx_dco_q_range[0], bbtx_dco_q_range[1]):
            print('')
            for bbtx_dco_i in range(bbtx_dco_i_range[0], bbtx_dco_i_range[1]):
                lo_leakage_magn = self._meas_lo_leakage_magn(dev, bbtx_dco_q, bbtx_dco_i, pol, rx_pol=rx_pol, reps=ADC_REP, n=4)
    ...
    def nr(self, dev, pol, rx_pol, i_dac_init, i_dac_0, i_dac_1, q_dac_init, q_dac_0, q_dac_1, step, target=None, num_of_meas=4, max_iter=60, slope_check=True, printit=True):
        ...
        while (not stop_iteration):
            ...
            if init_magn == 1000000:
                init_magn = lo_leakage_magn
            last_magn = lo_leakage_magn
            ...
            if max_lo_leakage_magn == 0:
                max_lo_leakage_magn = lo_leakage_magn
                if target == None:
                    target = round(max_lo_leakage_magn / 4.5, 3)
        ...
        min_lo_leakage_magn['init_magn'] = init_magn
        min_lo_leakage_magn['last_magn'] = last_magn
        min_lo_leakage_magn['n_iters'] = num_of_iterations
    ...
    def calibrate(self, dev, mode, pol, cross_pol=True, fine_tune=True):
        ...
        self._rx_setup(dev, pol, rx_pol, mode.lower())
        if pol != rx_pol:
            self._switch_lb(dev, pol, True)

        self._pre_calibration_settings(dev, pol, rx_pol)

        # Enable RX override
        if rx_pol == 'h':
            self._spi.wr(dev, 'sel_rx_override', 0x00)
            self._spi.set(dev, 'sel_rx_override', 0x04)
            self._spi.wr(dev, 'bf_en_rx_h_override', 0x0)
            self._spi.wr(dev, 'en_rx_h_override', 0x2ff)
        else:
            self._spi.wr(dev, 'sel_rx_override', 0x00)
            self._spi.set(dev, 'sel_rx_override', 0x40)
            self._spi.wr(dev, 'bf_en_rx_v_override', 0x0)
            self._spi.wr(dev, 'en_rx_v_override', 0x2ff)

        if cross_pol:
            com_gain_rx_vga = self._select_com_gain_rx(dev, pol, rx_pol, mode.lower())
        else:
            bbrx_iq_pga1_gain = self._select_pga1_gain_rx(dev, pol, rx_pol, mode.lower())
        #self.set_rx_gain(dev, rx_pol, 0x3f)
        # for nn in range(10):
        #     lo_leakage_magn = self._meas_lo_leakage_magn(dev, 0x3C7, 0x35F, pol, rx_pol=rx_pol, n=4)
        #     print('Optimal [0x3C7, 0x35F]: ', lo_leakage_magn)
        ...
        res = self.nr(dev, pol, rx_pol, bbtx_dco_i, 0, self.max_reg_value, bbtx_dco_q, 0, self.max_reg_value, self.step[0], num_of_meas=1, slope_check=False)
        print(res)
        ...
        print(res)
        if res['magn'] > 0.001:
            res = self.nr(dev, pol, rx_pol, res['i_dac'], 0, self.max_reg_value, res['q_dac'], 0, self.max_reg_value, self.step[2], target=0.0, max_iter=40, num_of_meas=2, slope_check=False)
        if res['magn'] <= min_lo_leakage_magn['magn']:
            min_lo_leakage_magn = res
        print(res)
        if pol != rx_pol:
            self.set_com_gain_rx(dev, rx_pol, 0x3f)
        else:
            self.set_bbrx_iq_pga1_gain(dev, rx_pol, 0x7)
        fine_tune_res = self.calibrate_fine_tune(dev, pol, rx_pol=rx_pol, search_offset=2, bbtx_dco_reg=self._merge_fields(min_lo_leakage_magn['i_dac'], min_lo_leakage_magn['q_dac']))
        print(fine_tune_res)
        result = {'reg_dco_q' : fine_tune_res['reg_dco_q'], 'reg_dco_i' : fine_tune_res['reg_dco_i'], 'lo_leakage_magn' : fine_tune_res['lo_leakage_magn']}

        if True:
            #self.set_rx_gain(dev, rx_pol, com_gain_rx_vga+2)
            if mode == 'BB':
                # Adjust IF DACs
                ...
                #else:
                #    self.set_bbrx_iq_pga1_gain(dev, rx_pol, 0x7)
                res = self.nr(dev, pol, rx_pol, bbtx_dco_i, 0, self.max_reg_value, bbtx_dco_q, 0, self.max_reg_value, self.step[1], target=0.0, num_of_meas=1, slope_check=False, max_iter=20)
                if pol != rx_pol:
                    self.set_com_gain_rx(dev, rx_pol, 0x3f)
                else:
                    self.set_bbrx_iq_pga1_gain(dev, rx_pol, 0x7)
                fine_tune_res = self.calibrate_fine_tune(dev, pol, rx_pol=rx_pol, search_offset=3, bbtx_dco_reg=self._merge_fields(res['i_dac'], res['q_dac']))
    ...
    def sweep(self, dev, mode, pol, gain_index, cross_pol, step=100, q_dac=[0,0x7ff], i_dac=[0,0x7ff], printit=False):
        """SWEEP2
        ...
        self._pre_calibration_settings(dev, pol, rx_pol, gain_index)
        # Enable RX override
        if rx_pol == 'h':
            self._spi.wr(dev, 'sel_rx_override', 0x00)
            self._spi.set(dev, 'sel_rx_override', 0x04)
        ...
        time_elapsed = end_time - init_time
        print('Time elapsed: {}s'.format(time_elapsed.seconds))
        magn = self._meas_lo_leakage_magn(dev, min_value['bbtx_dco_q'], min_value['bbtx_dco_i'], pol, rx_pol, test_data=False, n=1)
        return min_value
    ...
    def get_rx_gain(self, dev, rx_pol):
        rx_gain = self._chip.ram.rd(dev, 'rx_ram_'+rx_pol.lower(), 0)
        com_gain_rx_vga = (rx_gain >> 59) & 0b111111
        return com_gain_rx_vga

    def set_com_gain_rx(self, dev, rx_pol, com_gain_rx_vga):
        if com_gain_rx_vga > 0x3f:
            com_gain_rx_vga = 0x3f
        if com_gain_rx_vga < 0:
            com_gain_rx_vga = 0
        print('set_com_gain_rx', hex(com_gain_rx_vga))
        rx_gain = self._chip.ram.rd(dev, 'rx_ram_'+rx_pol.lower(), 0)
        rx_gain = (rx_gain & 0x7ffffffffffffff) + (com_gain_rx_vga << 59)
        self._chip.ram.wr(dev, 'rx_ram_'+rx_pol.lower(), 0, rx_gain)
        self._chip.rx.gain(dev, 0, 'R'+rx_pol.upper())

    def set_bbrx_iq_pga1_gain(self, dev, rx_pol, bbrx_iq_pga1_gain):
        if bbrx_iq_pga1_gain > 0x7:
            bbrx_iq_pga1_gain = 0x7
        if bbrx_iq_pga1_gain < 0:
            bbrx_iq_pga1_gain = 0
        print('set_bbrx_iq_pga1_gain', hex(bbrx_iq_pga1_gain))
        rx_gain = self._chip.ram.rd(dev, 'rx_ram_'+rx_pol.lower(), 0)
        rx_gain = (rx_gain & 0x0ffffffffffffff) + (bbrx_iq_pga1_gain << 56)
        self._chip.ram.wr(dev, 'rx_ram_'+rx_pol.lower(), 0, rx_gain)
        self._chip.rx.gain(dev, 0, 'R'+rx_pol.upper())
    ...
    def set_rx_gain(self, dev, rx_pol, com_gain_rx_vga):
        ...
        if com_gain_rx_vga < 0:
            com_gain_rx_vga = 0
        print('set_rx_gain', hex(com_gain_rx_vga))
        rx_gain = self._chip.ram.rd(dev, 'rx_ram_'+rx_pol.lower(), 0)
        rx_gain = (rx_gain & 0x7ffffffffffffff) + (com_gain_rx_vga << 59)
        self._chip.ram.wr(dev, 'rx_ram_'+rx_pol.lower(), 0, rx_gain)
        self._chip.rx.gain(dev, 0, 'R'+rx_pol.upper())
