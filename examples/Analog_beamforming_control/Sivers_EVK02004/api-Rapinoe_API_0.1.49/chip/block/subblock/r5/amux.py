...
class Amux():
    ...
    def __init__(self, spi):
        ...
        self.src = {'ADC DAC':              {'amux':0, 'amux8':0, 'synth':None, 'biastop':None, 'mult':1.0, 'offs':False},
                    ...
                    'BB V RX I FILT DC':    {'amux':6, 'amux8':0, 'synth':None, 'biastop':None, 'mult':2.0, 'offs':False},
                    'BB V RX Q FILT DC':    {'amux':6, 'amux8':1, 'synth':None, 'biastop':None, 'mult':2.0, 'offs':False},
                    'BB H RX I FILT DC':    {'amux':7, 'amux8':0, 'synth':None, 'biastop':None, 'mult':2.0, 'offs':False},
                    'BB H RX Q FILT DC':    {'amux':7, 'amux8':1, 'synth':None, 'biastop':None, 'mult':2.0, 'offs':False}
                   }
    ...
    def _set(self, dev, amux_src, amux8_src=None, synth_src=None, biastop_src=None):
        ...
        if ret != 'No source':
            ...
            if 'SW' in self._chip_info.get(dev)['workaround']:
                if (ret['amux'] == 7) and (ret['amux8'] == 0):
                    self.enable_dco_filter_I_H(dev)
                elif (ret['amux'] == 7) and (ret['amux8'] == 1):
                    self.enable_dco_filter_Q_H(dev)
                elif (ret['amux'] == 6) and (ret['amux8'] == 0):
                    self.enable_dco_filter_I_V(dev)
                elif (ret['amux'] == 6) and (ret['amux8'] == 1):
                    self.enable_dco_filter_Q_V(dev)
                else:
                    ...
                    self._spi.wr(dev, 'amux_ctrl_src', ((7-ret['amux']) << 4) + (7-ret['amux8']))
 ...
 ##################### Filter DCO src workaround #################################

    def enable_dco_filter_I_H(self, dev):
        self._spi.set(dev, 'amux_ctrl_cfg', 0x300000)
        self.set_amux(dev, 7)
        self.set_amux8(dev, 6)

    def enable_dco_filter_Q_H(self, dev):
        self._spi.set(dev, 'amux_ctrl_cfg', 0x300000)
        self.set_amux(dev, 7)
        self.set_amux8(dev, 7)

    def set_amux8(self, dev, amux8):
        self._spi.wr(dev, 'amux_ctrl_src', amux8)
        self._spi.wr(dev, 'amux_ctrl', 0x11)
        self._spi.wr(dev, 'amux_ctrl', 0x13)
        self._spi.wr(dev, 'amux_ctrl', 0x11)

    def set_amux(self, dev, amux):
        self._spi.wr(dev, 'amux_ctrl_src', (amux << 4))
        self._spi.wr(dev, 'amux_ctrl', 0x11)
        self._spi.wr(dev, 'amux_ctrl', 0x31)
        self._spi.wr(dev, 'amux_ctrl', 0x11)

    def enable_dco_filter_I_V(self, dev):
        self._spi.set(dev, 'amux_ctrl_cfg', 0x300000)
        self._spi.wr(dev, 'amux_ctrl_src', ((7-1) << 4) + (7-6))
        self._spi.wr(dev, 'amux_ctrl', 0x11)
        self._spi.wr(dev, 'amux_ctrl', 0x31)
        self._spi.wr(dev, 'amux_ctrl', 0x11)
        self._spi.wr(dev, 'amux_ctrl_src', (1 << 4) + 6)
        self._spi.wr(dev, 'amux_ctrl', 0x13)
        self._spi.wr(dev, 'amux_ctrl', 0x11)

    def enable_dco_filter_Q_V(self, dev):
        self._spi.set(dev, 'amux_ctrl_cfg', 0x300000)
        self._spi.wr(dev, 'amux_ctrl_src', ((7-1) << 4) + (7-7))
        self._spi.wr(dev, 'amux_ctrl', 0x11)
        self._spi.wr(dev, 'amux_ctrl', 0x31)
        self._spi.wr(dev, 'amux_ctrl', 0x11)
        self._spi.wr(dev, 'amux_ctrl_src', (1 << 4) + 7)
        self._spi.wr(dev, 'amux_ctrl', 0x13)
        self._spi.wr(dev, 'amux_ctrl', 0x11)
