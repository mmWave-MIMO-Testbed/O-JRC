...
class mimo_ofdm_comm_sim(gr.top_block, Qt.QWidget):
    ...
    def __init__(self):
        ...
        self.mimo_ofdm_jrc_socket_pdu_jrc_0 = mimo_ofdm_jrc.socket_pdu_jrc('UDP_SERVER', '', '52001', mtu_value)
        ...
        self.blocks_socket_pdu_1 = blocks.socket_pdu('UDP_CLIENT', '127.0.0.1', '52002', 5000)
...
stop_thread = False  # Global flag to stop the thread
...
def main(top_block_cls=mimo_ofdm_comm_sim, options=None):
    ...
    def probe_mtu_value():
        global stop_thread
        current_mtu = tb.mtu_value  # Initialize current_mtu with the initial value of mtu_value
        while not stop_thread:
            if tb.mtu_value != current_mtu:  # Check if mtu_value has changed
                current_mtu = tb.mtu_value
                tb.mimo_ofdm_jrc_socket_pdu_jrc_0.set_MTU(current_mtu)  # Call set_MTU with the new value
                print(f"New MTU value: {current_mtu}")  # Print the new MTU value
            else:
                print(f"Current MTU value: {current_mtu}")  # Print the current MTU value
            
            time.sleep(1)  # Sleep for a short period of time (e.g., 100 ms) before checking again
    
    mtu_value_thread = threading.Thread(target=probe_mtu_value)
    mtu_value_thread.start()
